...
def apply_normalize_shim(
    ...
    # Also normalize sharp_image so it's in the same [-1,1] range as image
    # (sharp_image is loaded with ToTensor → [0,1], same as image before shim)
    if "sharp_image" in batch["context"]:
        batch["context"]["sharp_image"] = normalize_image(
            batch["context"]["sharp_image"], mean, std
        )
    return batch
