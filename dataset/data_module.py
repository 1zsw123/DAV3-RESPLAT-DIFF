...
class DataLoaderStageCfg:
    ...
    seed: int | None
    num_val_samples: int = 1  # how many samples to run in val (default=1 for viz)
...
class DataModule(LightningDataModule):
        ...
        for dataset in datasets:
            ...
            data_loaders.append(
                DataLoader(
                    ...
                    timeout=600,  # raise error after 10 min instead of hanging on GPFS stall
                )
            )
    ...
    def val_dataloader(self):
        ...
        for dataset in datasets:
            ...
            data_loaders.append(
                DataLoader(
                    ValidationWrapper(dataset, self.data_loader_cfg.val.num_val_samples),
                    ...
                    timeout=600,  # same as train: prevent GPFS stall from hanging rank 0 and causing NCCL ALLREDUCE timeout on other ranks
                )
            )
        return data_loaders if len(data_loaders) > 1 else data_loaders[0]
