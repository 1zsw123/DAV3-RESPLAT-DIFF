...
class ViewSamplerBoundedCfg:
    ...
    min_context_views: int = 2  # randomly sample this many to num_context_views each step
...
class ViewSamplerBounded(ViewSampler[ViewSamplerBoundedCfg]):
    ...
    def sample(
        ...
        if self.cfg.num_context_views > 2:
            ...
            available = index_context_right - index_context_left - 1
            if available < num_extra_views:
                raise ValueError(
                    f"Not enough frames between context views to sample "
                    f"{num_extra_views} extra views (only {available} available). "
                    f"Increase max_distance_between_context_views or reduce num_context_views."
                )
            extra_views = []
            max_attempts = 10000
            for _ in range(max_attempts):
                ...
                if len(set(extra_views)) == num_extra_views:
                    break
            else:
                raise ValueError("Could not sample unique extra context views after max attempts.")
        else:
            extra_views = []
